...
private func toSignedInteger<T: SignedInteger>(_ value: Any?) -> T? {
	...
	if T.self ==   Int.self, let x = Int(exactly: number.intValue) {
		return T.init(x)
	}
	if T.self ==  Int8.self, let x = Int8(exactly: number.int8Value) {
		return T.init(x)
	}
	if T.self == Int16.self, let x = Int16(exactly: number.int16Value){
		return T.init(x)
	}
	if T.self == Int32.self, let x = Int32(exactly: number.int32Value) {
		return T.init(x)
	}
